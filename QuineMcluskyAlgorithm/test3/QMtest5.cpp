...
std::vector<std::vector<B_number> > p2_group;
std::vector<std::vector<B_number> > p3_group;
...
void create_table();
void create_p_group();
void create_p2_group();
void create_p3_group();
      ...
      for (size_t k = 0; k < p_group[i + 1].size(); k++) {
        if (p_group[i][j].dashes == p_group[i+1][k].dashes) {
          ...
          if (count_1s(temp_dashes) == 1) {
            ...
            if (tmp + 1> p2_group.size()) {
              p2_group.resize(tmp + 1);
            }
            ...
            p2_group[tmp].push_back(temp_num);
          }
        }
      }
    }
...
void create_p3_group() {
  ...
  B_number temp_num;
  unsigned temp_number, temp_dashes;
  for (size_t i = 0; i < p2_group.size() - 1; i++) {
    for (size_t j = 0; j < p2_group[i].size(); j++) {
      for (size_t k = 0; k < p2_group[i + 1].size(); k++) {
        if (p2_group[i][j].dashes == p2_group[i+1][k].dashes) {

          temp_number = p2_group[i][j].number & p2_group[i+1][k].number;
          temp_dashes = p2_group[i][j].number ^ p2_group[i+1][k].number;

          if (count_1s(temp_dashes) == 1) {
            temp_dashes ^= p2_group[i][j].dashes;

            p2_group[i][j].used = true;
            p2_group[i + 1][k].used = true;

            tmp = count_1s(temp_number);

            if (tmp + 1> p3_group.size()) {
              p3_group.resize(tmp + 1);
            }
            temp_num = init_B_number(temp_number, temp_dashes, true);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[0]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[1]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[2]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[3]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[0]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[1]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[2]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[3]);
            p3_group[tmp].push_back(temp_num);
          }
        }
      }
    }
...
void create_final_group() {
  ...
  B_number temp_num;
  unsigned temp_number, temp_dashes;
  for (size_t i = 0; i < p3_group.size() - 1; i++) {
    for (size_t j = 0; j < p3_group[i].size(); j++) {
      for (size_t k = 0; k < p3_group[i + 1].size(); k++) {
        if (p2_group[i][j].dashes == p3_group[i+1][k].dashes) {

          temp_number = p3_group[i][j].number & p3_group[i+1][k].number;
          temp_dashes = p3_group[i][j].number ^ p3_group[i+1][k].number;

          if (count_1s(temp_dashes) == 1) {
            temp_dashes ^= p3_group[i][j].dashes;

            p3_group[i][j].used = true;
            p3_group[i + 1][k].used = true;
            ...
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[2]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[3]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[4]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[5]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[6]);
            temp_num.used_numbers.push_back(p2_group[i][j].used_numbers[7]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[0]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[1]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[2]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[3]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[4]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[5]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[6]);
            temp_num.used_numbers.push_back(p2_group[i+1][k].used_numbers[7]);
            p3_group[tmp].push_back(temp_num);
          }
        }
      }
...
void create_used_num_group() {
  ...
  for (size_t i = 0; i < p3_group.size(); i++) {
    for (size_t j = 0; j < p3_group[i].size(); j++) {
      if (!p3_group[i][j].used) {
        for (size_t x = 0; x < p3_group[i][j].used_numbers.size(); x++) {
          used_numbers_group[p3_group[i][j].used_numbers[x]]++;
        }
      }
    }
  }
  for (size_t i = 0; i < p2_group.size(); i++) {
    for (size_t j = 0; j < p2_group[i].size(); j++) {
      if (!p2_group[i][j].used) {
        for (size_t x = 0; x < p2_group[i][j].used_numbers.size(); x++) {
          used_numbers_group[p2_group[i][j].used_numbers[x]]++;
        }
      }
    }
  }
...
void check_epi(){
  ...
  for (size_t i = 0; i < p3_group.size(); i++) {
    for (size_t j = 0; j < p3_group[i].size(); j++) {
      for (size_t k = 0; k < p3_group[i][j].used_numbers.size(); k++) {
        for (size_t x = 0; x < 64; x++) {
          if (used_numbers_group[x] == 1) {
            if (x == p3_group[i][j].used_numbers[k]) {
              p3_group[i][j].epi = true;
            }
          }
        }
      }
  ...
  for (size_t i = 0; i < p2_group.size(); i++) {
    for (size_t j = 0; j < p2_group[i].size(); j++) {
      for (size_t k = 0; k < p2_group[i][j].used_numbers.size(); k++) {
        for (size_t x = 0; x < 64; x++) {
          if (used_numbers_group[x] == 1) {
            if (x == p2_group[i][j].used_numbers[k]) {
              p2_group[i][j].epi = true;
            }
          }
        }
      }
...
void print_epi() {
  ...
  for (size_t i = 0; i < p3_group.size(); i++) {
    for (size_t j = 0; j < p3_group[i].size(); j++) {
      if (!p3_group[i][j].used && p3_group[i][j].epi == true) {
        print_p_binary(p3_group[i][j].number, p3_group[i][j].dashes);
        std::cout << " ";
      }
    }
  }
  for (size_t i = 0; i < p2_group.size(); i++) {
    for (size_t j = 0; j < p2_group[i].size(); j++) {
      if (!p2_group[i][j].used && p2_group[i][j].epi == true) {
        print_p_binary(p2_group[i][j].number, p2_group[i][j].dashes);
        std::cout << " ";
      }
    }
  }
...
void print_nepi() {
  ...
  for (size_t i = 0; i < p3_group.size(); i++) {
    for (size_t j = 0; j < p3_group[i].size(); j++) {
      if (!p3_group[i][j].used && p3_group[i][j].epi == false ) {
        print_p_binary(p3_group[i][j].number, p3_group[i][j].dashes);
        std::cout << " ";
      }
    }
  ...
  for (size_t i = 0; i < p2_group.size(); i++) {
    for (size_t j = 0; j < p2_group[i].size(); j++) {
      if (!p2_group[i][j].used && p2_group[i][j].epi == false ) {
        print_p_binary(p2_group[i][j].number, p2_group[i][j].dashes);
        std::cout << " ";
      }
    }
