...
std::vector<std::vector<B_number> > p_group2;
      ...
      for (size_t k = 0; k < p_group[i + 1].size(); k++) {
        if (p_group[i][j].dashes == p_group[i+1][k].dashes) {
          ...
          if (count_1s(temp_dashes) == 1) {
            ...
            if (tmp + 1> p_group2.size()) {
              p_group2.resize(tmp + 1);
            }
            ...
            p_group2[tmp].push_back(temp_num);
          }
        }
      }
    }
...
void create_final_group() {
  ...
  B_number temp_num;
  unsigned temp_number, temp_dashes;
  for (size_t i = 0; i < p_group2.size() - 1; i++) {
    for (size_t j = 0; j < p_group2[i].size(); j++) {
      for (size_t k = 0; k < p_group2[i + 1].size(); k++) {
        if (p_group2[i][j].dashes == p_group2[i+1][k].dashes) {

          temp_number = p_group2[i][j].number & p_group2[i+1][k].number;
          temp_dashes = p_group2[i][j].number ^ p_group2[i+1][k].number;

          if (count_1s(temp_dashes) == 1) {
            temp_dashes ^= p_group2[i][j].dashes;

            p_group2[i][j].used = true;
            p_group2[i + 1][k].used = true;
            ...
            temp_num = init_B_number(temp_number, temp_dashes, true);
            temp_num.used_numbers.push_back(p_group2[i][j].used_numbers[0]);
            temp_num.used_numbers.push_back(p_group2[i][j].used_numbers[1]);
            temp_num.used_numbers.push_back(p_group2[i][j].used_numbers[2]);
            temp_num.used_numbers.push_back(p_group2[i][j].used_numbers[3]);
            temp_num.used_numbers.push_back(p_group2[i+1][k].used_numbers[0]);
            temp_num.used_numbers.push_back(p_group2[i+1][k].used_numbers[1]);
            temp_num.used_numbers.push_back(p_group2[i+1][k].used_numbers[2]);
            temp_num.used_numbers.push_back(p_group2[i+1][k].used_numbers[3]);
...
void create_used_num_group() {
  ...
  for (size_t i = 0; i < p_group2.size(); i++) {
    for (size_t j = 0; j < p_group2[i].size(); j++) {
      if (!p_group2[i][j].used) {
        for (size_t x = 0; x < p_group2[i][j].used_numbers.size(); x++) {
          used_numbers_group[p_group2[i][j].used_numbers[x]]++;
          used_checked_number.push_back(p_group2[i][j]);
        }
      }
    }
  }
...
void check_epi(){
  ...
  for (size_t i = 0; i < p_group2.size(); i++) {
    for (size_t j = 0; j < p_group2[i].size(); j++) {
      for (size_t k = 0; k < p_group2[i][j].used_numbers.size(); k++) {
        for (size_t x = 0; x < 64; x++) {
          if (used_numbers_group[x] == 1) {
            if (x == p_group2[i][j].used_numbers[k]) {
              p_group2[i][j].epi = true;
            }
          }
        }
      }
...
void print_epi() {
  ...
  for (size_t i = 0; i < p_group2.size(); i++) {
    for (size_t j = 0; j < p_group2[i].size(); j++) {
      if (!p_group2[i][j].used && p_group2[i][j].epi == true) {
        print_p_binary(p_group2[i][j].number, p_group2[i][j].dashes);
        std::cout << " B ";
        printed_numbers.push_back(p_group2[i][j]);
      }
    }
  }
  for (size_t i = 0; i < p_group.size(); i++) {
...
void print_nepi() {
  ...
  for (size_t i = 0; i < p_group2.size(); i++) {
    for (size_t j = 0; j < p_group2[i].size(); j++) {
      if (!p_group2[i][j].used && p_group2[i][j].epi == false ) {
        print_p_binary(p_group2[i][j].number, p_group2[i][j].dashes);
        std::cout << " ";
        printed_numbers.push_back(p_group2[i][j]);
      }
